...
class OAuth2 {
	...
	const DEFAULT_AUTH_CODE_LIFETIME = 3600;//30
	...
	public function grantAccessToken(array $inputData = NULL, array $authHeaders = NULL) {
		...
		//hxd，设置return_uri
		//如果没有user_id，像“两条腿”那种，就不返回user_id了
		//这下面的为什么要这么设置，我也不知道，就是看到authoruze.php最后是这么返回的
		if($user_id) $result["query"]["user_id"]=$user_id;
		$result["fragment"]=$token;
		$this->doRedirectUriCallback($stored["redirect_uri"], $result);
	}
